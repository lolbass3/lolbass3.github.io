...
var BLVDDependenciesLoaded = BLVDDependenciesLoaded || false;
var BLVDTemplates = BLVDTemplates || false;

//todo:event handlers seem randomly scattered.
(function ($) {

    var BLVD = function($anchor, myPlaylist, userOptions){
        //Boulevard globals
        ...
        var EventManager, Layout, AlbumCoverManager, AlbumCover, LayoutManager, PlaylistInfo, TrackListManager, TrackInfoManager,
            PlaylistManager, BackgroundManager, GradientGenerator, DependencyLoader, TemplateManager;
...
//                    //todo: I should seriously consider just having a background container that this is added to
            ...
            function addOverlay() {
                var isDominantColorOverlay = options.overlayColor == 'dominant';

                //create the overlay iff the overlay hasn't been created yet
                if (!$background) {
                    createBackground('overlay-background');
                    displayBackground();
                }

                if (isDominantColorOverlay === true) {
                    setOverlayColor(rgbaSetOpacity(imageColors.dominant, options.overlayOpacity));
                }
                else {
                    setOverlayColor(options.overlayColor);
                }
            }
            ...
            function setOverlayColor(color) {
                ...
                var colors = typeof color === 'string' ? [color] : color,
                    overlayOpacity = options.overlayOpacity,
                    overlayColor, playerControlsColor, color1, color2;
                ...
                if (colors.length == 1) {
                    ...
                    playerControlsColor = rgbaSetOpacity(colors[0], .2);

                    setBackgroundColor(overlayColor);
                    setPlayerControlsColor(playerControlsColor);
                }
                else {
                    ...
                    var gradientParams = {
                        color1:color1,
                        color2:color2,
                        deg:options.gradientDegrees
                    };

                    var gradientCss = GradientGenerator.generate(options.gradientType, gradientParams);

                    $background.attr('style', gradientCss);
                }
            ...
            function setDominantColorBackground() {
                var processing = $.Deferred();

                createBackground('dominant-color-background');

                setBackgroundColor(imageColors.dominant);

                $.when(displayBackground()).done(function () {
                    processing.resolve();
                });
            ...
            function rgbToRgba(rgbString, opacity) {
                var rgb = rgbStringToArray(rgbString);

                return 'rgba(' + rgb.join(',') + ',' + opacity + ')';
            }

            function rgbaSetOpacity(rgbaString, opacity){

                var rgba = rgbStringToArray(rgbaString);
            ...
            function setImageBackgroundThenRunFunction(callback) {
                var processing = $.Deferred();
                $.when(setImageBackground(false)).done(function () {
                    callback();
                    fadeOutOldCanvas();
                    processing.resolve();
                });
            ...
            function overlay() {
                options.imageEffects = 'greyscale';
                return overlaySimple();
            }

            function overlaySimple() {
                return setImageBackgroundThenRunFunction(function () {
            ...
            function applyBackground() {
                var applyingBackground = false;

                switch (styleType) {
                    case 'dominant-color':
                        applyingBackground = setDominantColorBackground();
                        break;
                    case 'blur':
                        applyingBackground = setImageBackground();
                        break;
                    case 'blur-player':
                        applyingBackground = setImageBackground();
                        $.when(applyingBackground).done(manageGeneralBackground);
                        break;
                    case 'overlay-simple':
                        applyingBackground = overlaySimple();
                        break;
                    case 'overlay':
                        applyingBackground = overlay();
                        break;
                    case 'image':
                        applyingBackground = setImageBackground();
                        break;
        ...
        GradientGenerator = function () {
            function horizontal(color1, color2) {
                var css = '' +
                    'background-image: -webkit-linear-gradient(left, {color1} 0%, {color2} 100%); // Safari 5.1-6, Chrome 10+' +
                    'background-image: -o-linear-gradient(left, {color1} 0%, {color2} 100%); // Opera 12' +
                    'background-image: linear-gradient(to right, {color1} 0%, {color2} 100%); // Standard, IE10, Firefox 16+, Opera 12.10+, Safari 7+, Chrome 26+' +
                    'background-repeat: repeat-x;' +
                    'progid:DXImageTransform.Microsoft.gradient(startColorstr=\'{color1}\', endColorstr=\'{color2}\', GradientType=1);';

                return css.replace(/\{color1\}/g, color1).replace(/\{color2\}/g, color2)
            }

            function vertical(color1, color2) {
                var css = '' +
                    'background-image: -webkit-linear-gradient(top, {color1} 0%, {color2} 100%);  // Safari 5.1-6, Chrome 10+' +
                    'background-image:  -o-linear-gradient(top, {color1} 0%, {color2} 100%);  // Opera 12' +
                    'background-image: linear-gradient(to bottom, {color1} 0%, {color2} 100%); // Standard, IE10, Firefox 16+, Opera 12.10+, Safari 7+, Chrome 26+' +
                    'background-repeat: repeat-x;' +
            ...
            function directional(deg, color1, color2) {
                var css = '' +
                    'background-repeat: repeat-x;' +
                    'background-image: -webkit-linear-gradient({deg}, {color1}, {color2}); // Safari 5.1-6, Chrome 10+' +
                    'background-image: -o-linear-gradient({deg}, {color1}, {color2}); // Opera 12' +
                    'background-image: linear-gradient({deg}, {color1}, {color2}); // Standard, IE10, Firefox 16+, Opera 12.10+, Safari 7+, Chrome 26+';

                return css.replace(/\{color1\}/g, color1).replace(/\{color2\}/g, color2).replace(/\{deg\}/g, deg);
            }

            function radial(color1, color2) {
                var css = '' +
            ...
            function generate(type, params) {
                ...
                switch (type) {
                    case 'vertical':
                        css = vertical(params.color1, params.color2);
                        break;
                    case 'horizontal':
                        css = horizontal(params.color1, params.color2);
                        break;
                    case 'directional':
                        css = directional(params.deg, params.color1, params.color2);
                        break;
                    case 'radial':
                        css = radial(params.color1, params.color2);
                        break;
                }
        ...
        DependencyLoader = function () {
            ...
            function load(path) {
                ...
                if (!options.autoLoadDependencies || BLVDDependenciesLoaded === true) {
        ...
        TemplateManager = function () {
            ...
            this.load = function (path) {
                ...
                if (BLVDTemplates !== false) {
                    $templates = BLVDTemplates;
                    return true;
                }
        ...
        LayoutManager = function () {
            ...
            function initLayouts() {
                ...
                defineLayout('album-cover-and-list-fill-space', {
                    init:function(){
                        albumCoverManager.setAlbumCoverLayout('single-fill-space');
                ...
                defineLayout('info-and-album-cover', {
                    init:function () {
                        albumCoverManager.setAlbumCoverLayout('single');
                        trackInfoManager.init();
                    },
                    position:function () {
                        var margin = this.$inner.height() - albumCoverManager.getAlbumCoverSize() - 75;
                        this.$albumCover.css('margin-top', margin);

                        //we want the track info to be vertically centered with the album cover not, the window
                        margin = margin + albumCoverManager.getAlbumCoverSize() / 2 - this.$trackInfo.height() / 2;
                        this.$trackInfo.css('margin-top', margin);
                    }
                });

                defineLayout('info-and-list', {
                    init:function(){
                        trackInfoManager.init();
                        trackListManager.buildTrackList();
                    },
                    position:function(){
                        var margin = this.$inner.height() / 2 - this.$trackInfo.height();
                        this.$trackInfo.css('margin-top', margin);
                    }
                });
                ...
                defineLayout('info', {
                    ...
                    position:function () {
                        var margin = this.$inner.height() / 2 - this.$trackInfo.height() / 2;
                        this.$trackInfo.css('margin-top', margin);
                    }
                });

                defineLayout('album-cover-tiles', {
                    init:function () {
                        albumCoverManager.setAlbumCoverLayout('wall');
                    }
                });

                defineLayout('album-cover-wall', {
                    init:function () {
                        albumCoverManager.setAlbumCoverLayout('wall');
                    }
                });
            ...
            function hasLayoutType(type) {
                var layoutTypeFound = false;

                if (!options.layoutType)
                    return layoutTypeFound;

                $.each(options.layoutType.split(' '), function (i, layoutType) {

                    if (layoutType == type) {
                        layoutTypeFound = true;
                        return false;
                    }
            ...
            function addWatermark() {
                var prefix, watermarkId, css, cssId, $watermark;
                ...
                prefix = makePrefix();
                watermarkId = prefix + uniqueId();
                cssId = '#' + watermarkId;
                css = cssId + '{position: absolute;bottom: 100px;right: 20px;color: #fff; opacity: .2; font-size: 11px;' +
                    'text-decoration:none; }'
                    + cssId + ':hover{ color: #fff; opacity: .5;}' +
                    '.mini-height ' + cssId + '{bottom:70px;}' +
                    '[data-color="light-bg"] ' + cssId + '{color:#222;}';

                $('head').append('<style type="text/css">' + css + '</style>');
                // $watermark = $('<a id="' + watermarkId + '" href="http://boulevardjs.com" target="_blank">powered by Boulevard</a>');
                //
                // $self.find(cssSelector.playerControls).before($watermark);
            }
        ...
        //track info widget
        TrackInfoManager = function () {
            var isActive, $trackInfoWrapper, $title, $artist, isInitialized = false,
                sizes = {
                    150:'tiny',
                    250:'x-small',
                    400:'small',
            ...
            function init() {
                isActive = true;
                $trackInfoWrapper = $self.find('.track-info-wrapper'); //cssSelector
                $title = $trackInfoWrapper.find('.track-name').find('span');
                $artist = $trackInfoWrapper.find('.track-artist').find('span');
            ...
            function setInfo() {
                //because this function is getting called on playlistinit even for layouts that don't need it.
                if (!isActive)
                    return;
                ...
                $title.html(playlistInfo.trackName(current));
                $artist.html(playlistInfo.artist(current));

                setFontClass();
            }

            function setFontClass() {
                var width = $trackInfoWrapper.width(),
                    fontSizeClass = sizes['2000'];

                $.each(sizes, function (classWidth, sizeClass) {
                ...
                $trackInfoWrapper.attr('data-size', fontSizeClass);
            ...
            function bindEvents() {
                ...
                evtMgr.on('playlistInit playlistAdvance', function () {
                    //todo:this function should only run if this component is active
                    setInfo();
                });
...
//                    albumCover = albumCoversByPosition[positionOfClickedCover];
        ...
        AlbumCoverManager.prototype.setAlbumCoverWallLayout = function () {
            var self = this, coversLoaded = [];

            //we might need to repeat the playlist a couple of times to make sure the space is filled. This depends
            // entirely on the length of the playlist and the size of the container
            var height = this.$anchor.height(),
                width = this.$anchor.width(),
                tileHeight = width/ 5, //todo:this shouldnt be hardcoded
                numRowsNeeded = Math.ceil(height/tileHeight),
                totalTilesNeeded = numRowsNeeded * 5,
                numIterations = Math.ceil(totalTilesNeeded/myPlaylist.length),
                numTilesAdded = 0;


            var subtract = myPlaylist.length * numIterations -   totalTilesNeeded;

            for(var i = 0; i < numIterations; i++){
                $.each(myPlaylist, function (i) {

                    //we want the covers to fill the space entirely, but since we're using Math.ceil to round up
                    // (required so numIterations isn't a decimal), we may need to stop this loop manually.
                    // I.e. if we only need 30 tiles, the playlist is 11 songs long and num iterations is 3,
                    // then this loop will add 33 tiles if we don't stop it
                    if(numTilesAdded >= totalTilesNeeded)
                        return false;
                    ...
                    coversLoaded.push(albumCover.render());
                    numTilesAdded ++;
            ...
            $.when.apply($, coversLoaded).done(function () {
                self.sizeWallContainer();

                self.$anchor.parents('.nano').first().nanoScroller();
            });
        };

        AlbumCoverManager.prototype.sizeWallContainer = function () {
        ...
        AlbumCoverManager.prototype.bindPlayerEvents = function () {
            ...
            $(window).on('resize.' + id, function () {
                if (self.albumCoverLayout != 'wall')
                    self.positionAndSizeAlbumCovers();
                else self.sizeWallContainer();
            });
        };
        ...
        function showLoadingScreen(){
            $loading = $('<div class="blvd-loading"><div>Loading</div></div>').height($self.height());
...
//undo
    ...
    $.fn.ttwBoulevard = function (myPlaylist, userOptions) {
        return new BLVD(this, myPlaylist, userOptions);
    };
